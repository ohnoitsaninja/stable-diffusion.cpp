...
public:
    ...
    std::unique_ptr<GgmlBackendTensorResource> linear4(const GgmlBackendTensorResource& a_input,
                                                       float a,
                                                       const GgmlBackendTensorResource& b_input,
                                                       float b,
                                                       const GgmlBackendTensorResource& c_input,
                                                       float c,
                                                       const GgmlBackendTensorResource& d_input,
                                                       float d,
                                                       int n_threads,
                                                       const char* output_name) {
        auto get_graph = [&]() -> ggml_cgraph* {
            ggml_cgraph* gf = new_graph_custom(64);
            ggml_tensor* w = make_backend_input(a_input);
            ggml_tensor* x = make_backend_input(b_input);
            ggml_tensor* y = make_backend_input(c_input);
            ggml_tensor* z = make_backend_input(d_input);
            ggml_tensor* out = ggml_add(compute_ctx,
                                        ggml_add(compute_ctx,
                                                 ggml_scale(compute_ctx, w, a),
                                                 ggml_scale(compute_ctx, x, b)),
                                        ggml_add(compute_ctx,
                                                 ggml_scale(compute_ctx, y, c),
                                                 ggml_scale(compute_ctx, z, d)));
            ggml_build_forward_expand(gf, out);
            return gf;
        };
        return compute_to_backend_resource_handle(get_graph, n_threads, output_name);
    ...
    std::unique_ptr<GgmlBackendTensorResource> linear5(const GgmlBackendTensorResource& a_input,
                                                       float a,
                                                       const GgmlBackendTensorResource& b_input,
                                                       float b,
                                                       const GgmlBackendTensorResource& c_input,
                                                       ...
                                                       const GgmlBackendTensorResource& d_input,
                                                       float d,
                                                       const GgmlBackendTensorResource& e_input,
                                                       float e,
                                                       int n_threads,
                                                       const char* output_name) {
        auto get_graph = [&]() -> ggml_cgraph* {
            ggml_cgraph* gf = new_graph_custom(80);
            ggml_tensor* v = make_backend_input(a_input);
            ggml_tensor* w = make_backend_input(b_input);
            ggml_tensor* x = make_backend_input(c_input);
            ggml_tensor* y = make_backend_input(d_input);
            ggml_tensor* z = make_backend_input(e_input);
            ggml_tensor* out = ggml_add(compute_ctx,
                                        ggml_add(compute_ctx,
                                                 ggml_scale(compute_ctx, v, a),
                                                 ggml_scale(compute_ctx, w, b)),
                                        ggml_add(compute_ctx,
                                                 ggml_add(compute_ctx,
                                                          ggml_scale(compute_ctx, x, c),
                                                          ggml_scale(compute_ctx, y, d)),
                                                 ggml_scale(compute_ctx, z, e)));
            ggml_build_forward_expand(gf, out);
            return gf;
        };
        return compute_to_backend_resource_handle(get_graph, n_threads, output_name);
    ...
    std::unique_ptr<GgmlBackendTensorResource> denoised_from_cfg_model_output(const GgmlBackendTensorResource& model_output,
                                                                              const GgmlBackendTensorResource& x,
                                                                              float cfg_scale,
    ...
    std::unique_ptr<GgmlBackendTensorResource> denoised_from_cfg_outputs(const GgmlBackendTensorResource& cond_output,
    ...
    std::unique_ptr<GgmlBackendTensorResource> heun_update_from_derivatives(const GgmlBackendTensorResource& x,
                                                                            const GgmlBackendTensorResource& d,
                                                                            const GgmlBackendTensorResource& d2,
                                                                            float dt,
                                                                            int n_threads,
                                                                            const char* output_name) {
        auto get_graph = [&]() -> ggml_cgraph* {
            ggml_cgraph* gf = new_graph_custom(64);
            ggml_tensor* x_in = make_backend_input(x);
            ggml_tensor* d_in = make_backend_input(d);
            ggml_tensor* d2_in = make_backend_input(d2);
            ggml_tensor* avg = ggml_scale(compute_ctx,
                                          ggml_add(compute_ctx, d_in, d2_in),
                                          0.5f * dt);
            ggml_tensor* out = ggml_add(compute_ctx, x_in, avg);
            ggml_build_forward_expand(gf, out);
            return gf;
        };
        return compute_to_backend_resource_handle(get_graph, n_threads, output_name);
...
public:
    ...
    std::unique_ptr<GgmlBackendTensorResource> sample_kdiffusion_gpu_backend(
        sd_ctx_t* sd_ctx,
        const std::shared_ptr<DiffusionModel>& work_diffusion_model,
        sample_method_t method,
        ...
        if (method != EULER_SAMPLE_METHOD &&
            ...
            method != DPMPP2S_A_SAMPLE_METHOD &&
            method != DPMPP2M_SAMPLE_METHOD &&
            method != DPMPP2Mv2_SAMPLE_METHOD &&
            method != IPNDM_SAMPLE_METHOD &&
            method != IPNDM_V_SAMPLE_METHOD &&
            method != LCM_SAMPLE_METHOD &&
            method != DDIM_TRAILING_SAMPLE_METHOD &&
            method != TCD_SAMPLE_METHOD &&
            method != RES_MULTISTEP_SAMPLE_METHOD &&
            method != RES_2S_SAMPLE_METHOD &&
            method != ER_SDE_SAMPLE_METHOD &&
            method != DPMPP_SDE_SAMPLE_METHOD &&
            method != DPMPP_SDE_GPU_SAMPLE_METHOD &&
            method != DPMPP2M_SDE_SAMPLE_METHOD &&
            method != DPMPP2M_SDE_GPU_SAMPLE_METHOD &&
            method != DPMPP2M_SDE_HEUN_SAMPLE_METHOD &&
            method != DPMPP2M_SDE_HEUN_GPU_SAMPLE_METHOD &&
        ...
        if (method == DPMPP_SDE_SAMPLE_METHOD || method == DPMPP_SDE_GPU_SAMPLE_METHOD) {
            if (dpmpp_sde_r <= 0.0f || !std::isfinite(dpmpp_sde_r)) {
                LOG_ERROR("GPU DPM++ SDE sampler backend path requires r > 0");
                return nullptr;
            }
        }
        const bool is_sde_method =
            method == DPMPP_SDE_SAMPLE_METHOD ||
            method == DPMPP_SDE_GPU_SAMPLE_METHOD ||
            method == DPMPP2M_SDE_SAMPLE_METHOD ||
            method == DPMPP2M_SDE_GPU_SAMPLE_METHOD ||
            ...
            method == ER_SDE_SAMPLE_METHOD;
        if (is_sde_method) {
            dpmpp_offset_first_sigma_for_snr(sigmas, false);
        }
        ...
        if (use_diffusion_implicit_gemm) {
            LOG_INFO("GPU %s sampler backend using diffusion direct conv with implicit-GEMM CUDA kernel; set SDCPP_DISABLE_GPU_EULER_DIFFUSION_IMPLICIT_GEMM_CONV=1 to use legacy diffusion conv",
                     sampler_label);
        }
        ...
        float h_last = 0.0f;
        std::unique_ptr<GgmlBackendTensorResource> sde_denoised_1;
        std::unique_ptr<GgmlBackendTensorResource> sde_denoised_2;
        bool have_sde_denoised_1 = false;
        bool have_sde_denoised_2 = false;
        float sde_h_1 = 0.0f;
        float sde_h_2 = 0.0f;
        std::vector<std::unique_ptr<GgmlBackendTensorResource>> derivative_hist;
        bool have_res_old_sigma = false;
        float res_old_sigma_down = 0.0f;
        std::unique_ptr<GgmlBackendTensorResource> er_old_denoised_d;
        std::vector<float> er_lambdas;
        if (method == ER_SDE_SAMPLE_METHOD) {
            er_lambdas.resize(sigmas.size(), 0.0f);
            for (size_t j = 0; j < sigmas.size(); ++j) {
                er_lambdas[j] = std::exp(-dpmpp_sigma_to_half_log_snr(sigmas[j], false));
            }
        }
        std::vector<double> ddim_alphas_cumprod;
        std::vector<double> ddim_compvis_sigmas;
        if (method == DDIM_TRAILING_SAMPLE_METHOD || method == TCD_SAMPLE_METHOD) {
            constexpr float beta_start = 0.00085f;
            constexpr float beta_end = 0.0120f;
            ddim_alphas_cumprod.resize(TIMESTEPS);
            ddim_compvis_sigmas.resize(TIMESTEPS);
            for (int j = 0; j < TIMESTEPS; ++j) {
                ddim_alphas_cumprod[j] =
                    (j == 0 ? 1.0 : ddim_alphas_cumprod[j - 1]) *
                    (1.0 -
                     std::pow(std::sqrt(beta_start) +
                                  (std::sqrt(beta_end) - std::sqrt(beta_start)) *
                                      (static_cast<float>(j) / (TIMESTEPS - 1)),
                              2));
                ddim_compvis_sigmas[j] =
                    std::sqrt((1.0 - ddim_alphas_cumprod[j]) / ddim_alphas_cumprod[j]);
            }
        }
        auto current_latent_shape = [&]() -> std::vector<int64_t> {
            ggml_tensor* tensor = x != nullptr ? x->tensor : nullptr;
            if (tensor == nullptr) {
                return {};
            }
            return {tensor->ne[0], tensor->ne[1], tensor->ne[2], tensor->ne[3]};
        };
        auto make_step_noise = [&](int step_index, const char* name) -> std::unique_ptr<GgmlBackendTensorResource> {
            std::vector<int64_t> shape = current_latent_shape();
            if (shape.empty()) {
                return nullptr;
            }
            if (has_imported_step_noise) {
        ...
        auto phi1_fn = [](float t) -> float {
            ...
            return (std::exp(t) - 1.0f) / t;
        };
        auto phi2_fn = [&](float t) -> float {
            if (std::fabs(t) < 1e-6f) {
                return 0.5f + t / 6.0f + (t * t) / 24.0f;
            }
            return (phi1_fn(t) - 1.0f) / t;
        };
        auto push_derivative_hist = [&](std::unique_ptr<GgmlBackendTensorResource> d) {
            if (derivative_hist.size() == 3) {
                derivative_hist.erase(derivative_hist.begin());
            }
            derivative_hist.push_back(std::move(d));
        };
        auto er_noise_scaler = [](float value) -> float {
            if (value <= 0.0f) {
                return 0.0f;
            }
        ...
        for (int i = 0; i < steps; ++i) {
            ...
            float model_sigma = sigma;
            int ddim_timestep = 0;
            int ddim_prev_timestep = 0;
            int tcd_timestep_s = 0;
            if (method == DDIM_TRAILING_SAMPLE_METHOD) {
                ddim_timestep = static_cast<int>(std::round(TIMESTEPS - i * (static_cast<float>(TIMESTEPS) / steps))) - 1;
                ddim_timestep = std::max(0, std::min(TIMESTEPS - 1, ddim_timestep));
                ddim_prev_timestep = ddim_timestep - TIMESTEPS / steps;
                model_sigma = static_cast<float>(ddim_compvis_sigmas[ddim_timestep]);
                const float scale = (i == 0)
                                        ? std::sqrt(model_sigma * model_sigma + 1.0f) / model_sigma
                                        : std::sqrt(model_sigma * model_sigma + 1.0f);
                x = latent_runner.scale(*x, scale, n_threads, "gpu_ddim_trailing_pre_model_scale");
                if (x == nullptr || x->empty()) {
                    LOG_ERROR("GPU DDIM trailing sampler backend path failed pre-model scale at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
            } else if (method == TCD_SAMPLE_METHOD) {
                constexpr int original_steps = 50;
                ddim_timestep = TIMESTEPS - 1 - (TIMESTEPS / original_steps) *
                                                   static_cast<int>(std::floor(i * (static_cast<float>(original_steps) / steps)));
                ddim_timestep = std::max(0, std::min(TIMESTEPS - 1, ddim_timestep));
                ddim_prev_timestep = i >= steps - 1
                                         ? 0
                                         : TIMESTEPS - 1 - (TIMESTEPS / original_steps) *
                                                               static_cast<int>(std::floor((i + 1) * (static_cast<float>(original_steps) / steps)));
                ddim_prev_timestep = std::max(0, std::min(TIMESTEPS - 1, ddim_prev_timestep));
                tcd_timestep_s = static_cast<int>(std::floor((1.0f - eta) * ddim_prev_timestep));
                tcd_timestep_s = std::max(0, std::min(TIMESTEPS - 1, tcd_timestep_s));
                model_sigma = static_cast<float>(ddim_compvis_sigmas[ddim_timestep]);
                const float scale = (i == 0)
                                        ? std::sqrt(model_sigma * model_sigma + 1.0f) / model_sigma
                                        : std::sqrt(model_sigma * model_sigma + 1.0f);
                x = latent_runner.scale(*x, scale, n_threads, "gpu_tcd_pre_model_scale");
                if (x == nullptr || x->empty()) {
                    LOG_ERROR("GPU TCD sampler backend path failed pre-model scale at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
            }
            const bool trace_step = trace_euler_parity_step_enabled(i, steps);
            std::vector<float> scaling = denoiser->get_scalings(model_sigma);
            GGML_ASSERT(scaling.size() == 3);
            const float c_skip = scaling[0];
            const float c_out  = scaling[1];
            const float c_in   = scaling[2];
            ...
            if (method == EULER_SAMPLE_METHOD) {
            ...
            } else if (method == EULER_A_SAMPLE_METHOD) {
                auto denoised = make_cfg_denoised(*model_output,
                                                  separate_uncond_output.get(),
                                                  model_output_is_batched_cfg,
                                                  *x,
                                                  c_out,
                                                  c_skip,
                                                  "gpu_euler_a_denoised");
                if (denoised == nullptr || denoised->empty()) {
                    LOG_ERROR("GPU Euler A sampler backend path failed denoised tensor at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                auto ancestral = get_ancestral_step(sigma, sigma_next, eta);
                const float sigma_down = ancestral.first;
                const float sigma_up = ancestral.second;
                next_x = latent_runner.euler_update(*x,
                                                    *denoised,
                                                    sigma,
                                                    sigma_down,
                                                    n_threads,
                                                    "gpu_euler_a_update_to_sigma_down");
                if (next_x == nullptr || next_x->empty()) {
                    LOG_ERROR("GPU Euler A sampler backend path failed deterministic update at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                if (sigma_next > 0.0f && sigma_up > 0.0f) {
                    auto noise = make_step_noise(i, "gpu_euler_a_step_noise");
                    if (noise == nullptr || noise->empty()) {
                        LOG_ERROR("GPU Euler A sampler backend path failed CUDA noise at step %d", i + 1);
                        return nullptr;
                    }
                    next_x = latent_runner.add_scaled(*next_x,
                                                      *noise,
                                                      sigma_up,
                                                      n_threads,
                                                      "gpu_euler_a_add_noise");
                }
            } else if (method == DPMPP2S_A_SAMPLE_METHOD) {
                auto denoised = make_cfg_denoised(*model_output,
                                                  separate_uncond_output.get(),
                                                  model_output_is_batched_cfg,
                                                  *x,
                                                  c_out,
                                                  c_skip,
                                                  "gpu_dpmpp_2s_a_denoised");
                if (denoised == nullptr || denoised->empty()) {
                    LOG_ERROR("GPU DPM++ 2S A sampler backend path failed first denoised tensor at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                auto ancestral = get_ancestral_step(sigma, sigma_next, eta);
                ...
                const float sigma_up = ancestral.second;
                if (sigma_down == 0.0f) {
                    next_x = std::move(denoised);
                } else {
                    const float t = -std::log(sigma);
                    const float t_next = -std::log(sigma_down);
                    const float h = t_next - t;
                    const float half_exp = std::exp(-0.5f * h);
                    auto x2 = latent_runner.linear2(*x,
                                                    half_exp,
                                                    *denoised,
                                                    1.0f - half_exp,
                                                    n_threads,
                    ...
                    if (x2 == nullptr || x2->empty()) {
                        LOG_ERROR("GPU DPM++ 2S A sampler backend path failed midpoint latent at step %d", i + 1);
                        return nullptr;
                    }
                    backend_post_graphs += 1;

                    std::vector<float> scaling2 = denoiser->get_scalings(sigma_next);
                    ...
                    auto noised_input2 = latent_runner.scale(*x2,
                                                             c_in2,
                                                             n_threads,
                                                             "gpu_dpmpp_2s_a_noised_input2");
                    if (noised_input2 == nullptr || noised_input2->empty()) {
                        LOG_ERROR("GPU DPM++ 2S A sampler backend path failed midpoint scaled input at step %d", i + 1);
                        return nullptr;
                    }
                    backend_post_graphs += 1;

                    std::vector<float> timesteps_vec2 = prepare_sample_timesteps(sigma_next, shifted_timestep);
                    ...
                    if (use_dual_branch_cfg) {
                        ...
                        if (batched_noised_input2 == nullptr || batched_noised_input2->empty()) {
                            LOG_ERROR("GPU DPM++ 2S A sampler backend path failed midpoint batched input at step %d", i + 1);
                            return nullptr;
                        }
                        backend_post_graphs += 1;
                    ...
                    auto denoised2 = make_cfg_denoised(*model_output2,
                                                       ...
                                                       *x2,
                                                       c_out2,
                                                       c_skip2,
                                                       "gpu_dpmpp_2s_a_denoised2");
                    if (denoised2 == nullptr || denoised2->empty()) {
                        LOG_ERROR("GPU DPM++ 2S A sampler backend path failed midpoint denoised tensor at step %d", i + 1);
                        return nullptr;
                    }
                    backend_post_graphs += 1;
                    const float full_exp = std::exp(-h);
                    next_x = latent_runner.linear2(*x,
                                                   full_exp,
                                                   *denoised2,
                                                   1.0f - full_exp,
                                                   n_threads,
                                                   "gpu_dpmpp_2s_a_update");
                }
                if (next_x != nullptr && !next_x->empty() && sigma_next > 0.0f && sigma_up > 0.0f) {
                    auto noise = make_step_noise(i, "gpu_dpmpp_2s_a_step_noise");
                    if (noise == nullptr || noise->empty()) {
                        LOG_ERROR("GPU DPM++ 2S A sampler backend path failed CUDA noise at step %d", i + 1);
                        return nullptr;
                    }
                    next_x = latent_runner.add_scaled(*next_x,
                                                      *noise,
                                                      sigma_up,
                                                      n_threads,
                                                      "gpu_dpmpp_2s_a_add_noise");
                }
            } else if (method == DPMPP_SDE_SAMPLE_METHOD ||
                       method == DPMPP_SDE_GPU_SAMPLE_METHOD) {
                auto denoised = make_cfg_denoised(*model_output,
                                                  separate_uncond_output.get(),
                                                  model_output_is_batched_cfg,
                                                  *x,
                                                  c_out,
                                                  c_skip,
                                                  "gpu_dpmpp_sde_denoised");
                if (denoised == nullptr || denoised->empty()) {
                    LOG_ERROR("GPU DPM++ SDE sampler backend path failed first denoised tensor at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                if (sigma_next == 0.0f) {
                    ...
                    const float lambda_s = dpmpp_sigma_to_half_log_snr(sigma, false);
                    const float lambda_t = dpmpp_sigma_to_half_log_snr(sigma_next, false);
                    const float h = lambda_t - lambda_s;
                    const float lambda_s_1 = lambda_s + dpmpp_sde_r * h;
                    const float fac = 1.0f / (2.0f * dpmpp_sde_r);
                    const float sigma_s_1 = dpmpp_half_log_snr_to_sigma(lambda_s_1, false);
                    const float alpha_s = sigma * std::exp(lambda_s);
                    const float alpha_s_1 = sigma_s_1 * std::exp(lambda_s_1);
                    const float alpha_t = sigma_next * std::exp(lambda_t);

                    auto ancestral_1 = get_ancestral_step(std::exp(-lambda_s), std::exp(-lambda_s_1), eta);
                    const float sigma_down_1 = ancestral_1.first;
                    const float sigma_up_1 = ancestral_1.second;
                    if (sigma_down_1 <= 0.0f || !std::isfinite(sigma_down_1)) {
                        LOG_ERROR("GPU DPM++ SDE sampler backend path produced invalid intermediate sigma at step %d", i + 1);
                        return nullptr;
                    }
                    const float h_adjusted_1 = -std::log(sigma_down_1) - lambda_s;
                    auto x2 = latent_runner.linear2(*x,
                                                    (alpha_s_1 / alpha_s) * std::exp(-h_adjusted_1),
                                                    *denoised,
                                                    -alpha_s_1 * std::expm1(-h_adjusted_1),
                                                    n_threads,
                                                    "gpu_dpmpp_sde_x2");
                    if (x2 == nullptr || x2->empty()) {
                        LOG_ERROR("GPU DPM++ SDE sampler backend path failed intermediate latent at step %d", i + 1);
                        return nullptr;
                    }
                    backend_post_graphs += 1;
                    if (eta > 0.0f && s_noise > 0.0f && sigma_up_1 > 0.0f) {
                        auto noise = make_step_noise(i * 2, "gpu_dpmpp_sde_x2_noise");
                        if (noise == nullptr || noise->empty()) {
                            LOG_ERROR("GPU DPM++ SDE sampler backend path failed intermediate CUDA noise at step %d", i + 1);
                            return nullptr;
                        }
                        x2 = latent_runner.add_scaled(*x2,
                                                      *noise,
                                                      alpha_s_1 * s_noise * sigma_up_1,
                                                      n_threads,
                                                      "gpu_dpmpp_sde_x2_add_noise");
                        if (x2 == nullptr || x2->empty()) {
                            return nullptr;
                        }
                        backend_post_graphs += 1;
                    ...
                    std::vector<float> scaling2 = denoiser->get_scalings(sigma_s_1);
                    GGML_ASSERT(scaling2.size() == 3);
                    const float c_skip2 = scaling2[0];
                    const float c_out2  = scaling2[1];
                    const float c_in2   = scaling2[2];
                    auto noised_input2 = latent_runner.scale(*x2,
                                                             c_in2,
                                                             n_threads,
                                                             "gpu_dpmpp_sde_noised_input2");
                    if (noised_input2 == nullptr || noised_input2->empty()) {
                        LOG_ERROR("GPU DPM++ SDE sampler backend path failed intermediate scaled input at step %d", i + 1);
                        return nullptr;
                    }
                    backend_post_graphs += 1;
                    std::vector<float> timesteps_vec2 = prepare_sample_timesteps(sigma_s_1, shifted_timestep);
                    sd::Tensor<float> timesteps_tensor2({static_cast<int64_t>(timesteps_vec2.size())}, timesteps_vec2);
                    sd::Tensor<float> guidance_tensor2({1}, std::vector<float>{guidance.distilled_guidance});
                    std::unique_ptr<GgmlBackendTensorResource> model_output2;
                    std::unique_ptr<GgmlBackendTensorResource> separate_uncond_output2;
                    ...
                    auto denoised2 = make_cfg_denoised(*model_output2,
                                                       ...
                                                       *x2,
                                                       c_out2,
                                                       c_skip2,
                                                       "gpu_dpmpp_sde_denoised2");
                    if (denoised2 == nullptr || denoised2->empty()) {
                        return nullptr;
                    }
                    backend_post_graphs += 1;

                    auto ancestral_t = get_ancestral_step(std::exp(-lambda_s), std::exp(-lambda_t), eta);
                    const float sigma_down_t = ancestral_t.first;
                    const float sigma_up_t = ancestral_t.second;
                    if (sigma_down_t <= 0.0f || !std::isfinite(sigma_down_t)) {
                        LOG_ERROR("GPU DPM++ SDE sampler backend path produced invalid target sigma at step %d", i + 1);
                        return nullptr;
                    }
                    const float h_adjusted_t = -std::log(sigma_down_t) - lambda_s;
                    const float denoised_coeff = -alpha_t * std::expm1(-h_adjusted_t);
                    next_x = latent_runner.linear3(*x,
                                                   (alpha_t / alpha_s) * std::exp(-h_adjusted_t),
                                                   *denoised,
                                                   denoised_coeff * (1.0f - fac),
                                                   *denoised2,
                                                   denoised_coeff * fac,
                                                   n_threads,
                                                   "gpu_dpmpp_sde_update");
                    if (next_x != nullptr && !next_x->empty() && eta > 0.0f && s_noise > 0.0f && sigma_up_t > 0.0f) {
                        auto noise = make_step_noise(i * 2 + 1, "gpu_dpmpp_sde_step_noise");
                        if (noise == nullptr || noise->empty()) {
                            LOG_ERROR("GPU DPM++ SDE sampler backend path failed target CUDA noise at step %d", i + 1);
                            return nullptr;
                        }
                        next_x = latent_runner.add_scaled(*next_x,
                                                          *noise,
                                                          alpha_t * s_noise * sigma_up_t,
                                                          n_threads,
                                                          "gpu_dpmpp_sde_add_noise");
                    }
                }
            } else if (method == DPMPP2M_SDE_SAMPLE_METHOD ||
                       method == DPMPP2M_SDE_GPU_SAMPLE_METHOD ||
                       method == DPMPP2M_SDE_HEUN_SAMPLE_METHOD ||
                       method == DPMPP2M_SDE_HEUN_GPU_SAMPLE_METHOD) {
                const dpmpp_sde_solver_t solver_type =
                    (method == DPMPP2M_SDE_HEUN_SAMPLE_METHOD || method == DPMPP2M_SDE_HEUN_GPU_SAMPLE_METHOD)
                ...
                auto denoised = make_cfg_denoised(*model_output,
                                                  ...
                                                  *x,
                                                  c_out,
                                                  c_skip,
                                                  "gpu_dpmpp_2m_sde_denoised");
                if (denoised == nullptr || denoised->empty()) {
                    LOG_ERROR("GPU DPM++ 2M SDE sampler backend path failed denoised tensor at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                if (sigma_next == 0.0f) {
                    ...
                    const float alpha_t = sigma_next * std::exp(lambda_t);
                    const float x_coeff = (sigma_next / sigma) * std::exp(-h * eta);
                    const float denoised_coeff = alpha_t * (-std::expm1(-h_eta));
                    float corr_coeff = 0.0f;
                    if (have_old_denoised && h != 0.0f && h_last != 0.0f) {
                        const float r = h_last / h;
                        corr_coeff = solver_type == DPMPP_SDE_SOLVER_HEUN
                                         ? alpha_t * dpmpp_phi2(h_eta) * (1.0f / r)
                                         : 0.5f * alpha_t * (-std::expm1(-h_eta)) * (1.0f / r);
                    }
                    if (corr_coeff != 0.0f && old_denoised != nullptr && !old_denoised->empty()) {
                        next_x = latent_runner.linear3(*x,
                                                       x_coeff,
                                                       *denoised,
                                                       denoised_coeff + corr_coeff,
                                                       *old_denoised,
                                                       -corr_coeff,
                                                       n_threads,
                                                       "gpu_dpmpp_2m_sde_multistep_update");
                    } else {
                        next_x = latent_runner.linear2(*x,
                                                       x_coeff,
                                                       *denoised,
                                                       denoised_coeff,
                                                       n_threads,
                                                       "gpu_dpmpp_2m_sde_update");
                    }
                    const float noise_scale = dpmpp_sde_noise_scale(h, eta, s_noise, sigma_next);
                    if (next_x != nullptr && !next_x->empty() && noise_scale > 0.0f) {
                        auto noise = make_step_noise(i, "gpu_dpmpp_2m_sde_step_noise");
                        if (noise == nullptr || noise->empty()) {
                            LOG_ERROR("GPU DPM++ 2M SDE sampler backend path failed CUDA noise at step %d", i + 1);
                            return nullptr;
                        }
                        next_x = latent_runner.add_scaled(*next_x,
                                                          *noise,
                                                          noise_scale,
                                                          n_threads,
                                                          "gpu_dpmpp_2m_sde_add_noise");
                    }
                    h_last = h;
                }
                old_denoised = std::move(denoised);
                have_old_denoised = true;
            } else if (method == DPMPP3M_SDE_SAMPLE_METHOD ||
                       method == DPMPP3M_SDE_GPU_SAMPLE_METHOD) {
                auto denoised = make_cfg_denoised(*model_output,
                                                  separate_uncond_output.get(),
                                                  model_output_is_batched_cfg,
                                                  *x,
                                                  c_out,
                                                  c_skip,
                                                  "gpu_dpmpp_3m_sde_denoised");
                if (denoised == nullptr || denoised->empty()) {
                    LOG_ERROR("GPU DPM++ 3M SDE sampler backend path failed denoised tensor at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                if (sigma_next == 0.0f) {
                    ...
                    if (have_sde_denoised_2 && h != 0.0f && sde_h_1 != 0.0f && sde_h_2 != 0.0f) {
                        const float r0 = sde_h_1 / h;
                        const float r1 = sde_h_2 / h;
                        const float denom = r0 + r1;
                        if (r0 != 0.0f && r1 != 0.0f && denom != 0.0f) {
                            const float p = alpha_t * dpmpp_phi2(h_eta);
                            const float q = alpha_t * dpmpp_phi3(h_eta);
                            const float d10_den = 1.0f / r0;
                            const float d10_d1 = -1.0f / r0;
                            const float d11_d1 = 1.0f / r1;
                            const float d11_d2 = -1.0f / r1;
                            const float den_d1 = d10_den * (1.0f + r0 / denom);
                            const float d1_d1 = d10_d1 * (1.0f + r0 / denom) - d11_d1 * (r0 / denom);
                            const float d2_d1 = -d11_d2 * (r0 / denom);
                            const float den_d2 = d10_den / denom;
                            const float d1_d2 = (d10_d1 - d11_d1) / denom;
                            const float d2_d2 = (-d11_d2) / denom;
                            next_x = latent_runner.linear4(*x,
                                                           x_coeff,
                                                           *denoised,
                                                           denoised_coeff + p * den_d1 - q * den_d2,
                                                           *sde_denoised_1,
                                                           p * d1_d1 - q * d1_d2,
                                                           *sde_denoised_2,
                                                           p * d2_d1 - q * d2_d2,
                                                           n_threads,
                                                           "gpu_dpmpp_3m_sde_third_order_update");
                        }
                    }
                    if (next_x == nullptr && have_sde_denoised_1 && h != 0.0f && sde_h_1 != 0.0f) {
                        const float r = sde_h_1 / h;
                        const float corr = alpha_t * dpmpp_phi2(h_eta) * (1.0f / r);
                        next_x = latent_runner.linear3(*x,
                                                       x_coeff,
                                                       *denoised,
                                                       denoised_coeff + corr,
                                                       *sde_denoised_1,
                                                       -corr,
                                                       n_threads,
                                                       "gpu_dpmpp_3m_sde_second_order_update");
                    }
                    if (next_x == nullptr) {
                        next_x = latent_runner.linear2(*x,
                                                       x_coeff,
                                                       *denoised,
                                                       denoised_coeff,
                                                       n_threads,
                                                       "gpu_dpmpp_3m_sde_first_order_update");
                    }
                    const float noise_scale = dpmpp_sde_noise_scale(h, eta, s_noise, sigma_next);
                    if (next_x != nullptr && !next_x->empty() && noise_scale > 0.0f) {
                        auto noise = make_step_noise(i, "gpu_dpmpp_3m_sde_step_noise");
                        if (noise == nullptr || noise->empty()) {
                            LOG_ERROR("GPU DPM++ 3M SDE sampler backend path failed CUDA noise at step %d", i + 1);
                            return nullptr;
                        }
                        next_x = latent_runner.add_scaled(*next_x,
                                                          *noise,
                                                          noise_scale,
                                                          n_threads,
                                                          "gpu_dpmpp_3m_sde_add_noise");
                    }
                    sde_h_2 = sde_h_1;
                    sde_h_1 = h;
                }
                sde_denoised_2 = std::move(sde_denoised_1);
                sde_denoised_1 = std::move(denoised);
                have_sde_denoised_2 = have_sde_denoised_1;
                have_sde_denoised_1 = true;
            } else if (method == LCM_SAMPLE_METHOD) {
                auto denoised = make_cfg_denoised(*model_output,
                                                  separate_uncond_output.get(),
                                                  model_output_is_batched_cfg,
                                                  *x,
                                                  ...
                                                  "gpu_lcm_denoised");
                if (denoised == nullptr || denoised->empty()) {
                    LOG_ERROR("GPU LCM sampler backend path failed denoised tensor at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                next_x = std::move(denoised);
                if (sigma_next > 0.0f) {
                    auto noise = make_step_noise(i, "gpu_lcm_step_noise");
                    if (noise == nullptr || noise->empty()) {
                        LOG_ERROR("GPU LCM sampler backend path failed CUDA noise at step %d", i + 1);
                        return nullptr;
                    }
                    next_x = latent_runner.add_scaled(*next_x,
                                                      *noise,
            ...
            } else if (method == IPNDM_SAMPLE_METHOD ||
                       method == IPNDM_V_SAMPLE_METHOD) {
                auto denoised = make_cfg_denoised(*model_output,
                                                  separate_uncond_output.get(),
                                                  model_output_is_batched_cfg,
                                                  *x,
                                                  c_out,
                                                  c_skip,
                                                  method == IPNDM_SAMPLE_METHOD ? "gpu_ipndm_denoised" : "gpu_ipndm_v_denoised");
                if (denoised == nullptr || denoised->empty()) {
                    LOG_ERROR("GPU %s sampler backend path failed denoised tensor at step %d", sampler_label, i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                auto d_cur = latent_runner.euler_derivative_from_denoised(*x,
                                                                          *denoised,
                                                                          sigma,
                                                                          n_threads,
                                                                          method == IPNDM_SAMPLE_METHOD ? "gpu_ipndm_derivative" : "gpu_ipndm_v_derivative");
                if (d_cur == nullptr || d_cur->empty()) {
                    LOG_ERROR("GPU %s sampler backend path failed derivative at step %d", sampler_label, i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                const int order = std::min(4, i + 1);
                const float h_n = sigma_next - sigma;
                if (order == 1) {
                    next_x = latent_runner.add_scaled(*x,
                                                      *d_cur,
                                                      h_n,
                                                      n_threads,
                                                      method == IPNDM_SAMPLE_METHOD ? "gpu_ipndm_order1_update" : "gpu_ipndm_v_order1_update");
                } else if (order == 2 && !derivative_hist.empty()) {
                    const float h_n_1 = (method == IPNDM_V_SAMPLE_METHOD && i > 0) ? (sigma - sigmas[i - 1]) : h_n;
                    const float ratio = (method == IPNDM_V_SAMPLE_METHOD && h_n_1 != 0.0f) ? (h_n / h_n_1) : 1.0f;
                    const float d_coeff = method == IPNDM_V_SAMPLE_METHOD ? 0.5f * (2.0f + ratio) * h_n : 1.5f * h_n;
                    const float h_coeff = method == IPNDM_V_SAMPLE_METHOD ? -0.5f * ratio * h_n : -0.5f * h_n;
                    next_x = latent_runner.linear3(*x,
                                                   1.0f,
                                                   *d_cur,
                                                   d_coeff,
                                                   *derivative_hist.back(),
                                                   h_coeff,
                                                   n_threads,
                                                   method == IPNDM_SAMPLE_METHOD ? "gpu_ipndm_order2_update" : "gpu_ipndm_v_order2_update");
                } else if (order == 3 && derivative_hist.size() >= 2) {
                    next_x = latent_runner.linear4(*x,
                                                   1.0f,
                                                   *d_cur,
                                                   (23.0f / 12.0f) * h_n,
                                                   *derivative_hist[derivative_hist.size() - 1],
                                                   (-16.0f / 12.0f) * h_n,
                                                   *derivative_hist[derivative_hist.size() - 2],
                                                   (5.0f / 12.0f) * h_n,
                                                   n_threads,
                                                   method == IPNDM_SAMPLE_METHOD ? "gpu_ipndm_order3_update" : "gpu_ipndm_v_order3_update");
                } else if (order == 4 && derivative_hist.size() >= 3) {
                    next_x = latent_runner.linear5(*x,
                                                   1.0f,
                                                   *d_cur,
                                                   (55.0f / 24.0f) * h_n,
                                                   *derivative_hist[derivative_hist.size() - 1],
                                                   ...
                                                   *derivative_hist[derivative_hist.size() - 2],
                                                   (37.0f / 24.0f) * h_n,
                                                   *derivative_hist[derivative_hist.size() - 3],
                                                   (-9.0f / 24.0f) * h_n,
                                                   n_threads,
                                                   method == IPNDM_SAMPLE_METHOD ? "gpu_ipndm_order4_update" : "gpu_ipndm_v_order4_update");
                }
                push_derivative_hist(std::move(d_cur));
            } else if (method == RES_MULTISTEP_SAMPLE_METHOD) {
                auto denoised = make_cfg_denoised(*model_output,
                                                  separate_uncond_output.get(),
                                                  model_output_is_batched_cfg,
                                                  *x,
                                                  c_out,
                                                  c_skip,
                                                  "gpu_res_multistep_denoised");
                if (denoised == nullptr || denoised->empty()) {
                    LOG_ERROR("GPU RES multistep sampler backend path failed denoised tensor at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                auto ancestral = get_ancestral_step(sigma, sigma_next, eta);
                const float sigma_down = ancestral.first;
                const float sigma_up = ancestral.second;
                if (sigma_down == 0.0f || !have_res_old_sigma || old_denoised == nullptr || old_denoised->empty()) {
                    next_x = latent_runner.euler_update(*x,
                                                        *denoised,
                                                        sigma,
                                                        sigma_down,
                                                        n_threads,
                                                        "gpu_res_multistep_euler_update");
                } else {
                    const float t = -std::log(sigma);
                    const float t_old = -std::log(res_old_sigma_down);
                    const float t_next = -std::log(sigma_down);
                    const float t_prev = -std::log(sigmas[i - 1]);
                    const float h = t_next - t;
                    const float c2_ratio = h != 0.0f ? (t_prev - t_old) / h : 0.0f;
                    float b1 = 0.0f;
                    float b2 = 0.0f;
                    if (c2_ratio != 0.0f) {
                        const float phi1_val = phi1_fn(-h);
                        const float phi2_val = phi2_fn(-h);
                        b1 = phi1_val - phi2_val / c2_ratio;
                        b2 = phi2_val / c2_ratio;
                    }
                    if (!std::isfinite(b1)) {
                        b1 = 0.0f;
                    }
                    ...
                    next_x = latent_runner.linear3(*x,
                                                   ...
                                                   *old_denoised,
                                                   h * b2,
                                                   n_threads,
                                                   "gpu_res_multistep_update");
                }
                if (next_x != nullptr && !next_x->empty() && sigma_next > 0.0f && sigma_up > 0.0f) {
                    auto noise = make_step_noise(i, "gpu_res_multistep_step_noise");
                    if (noise == nullptr || noise->empty()) {
                        LOG_ERROR("GPU RES multistep sampler backend path failed CUDA noise at step %d", i + 1);
                        return nullptr;
                    }
                    next_x = latent_runner.add_scaled(*next_x,
                                                      *noise,
                                                      sigma_up,
                                                      n_threads,
                                                      "gpu_res_multistep_add_noise");
                }
                old_denoised = std::move(denoised);
                res_old_sigma_down = sigma_down;
                have_res_old_sigma = true;
            ...
            } else if (method == DDIM_TRAILING_SAMPLE_METHOD ||
                       method == TCD_SAMPLE_METHOD) {
                auto denoised = make_cfg_denoised(*model_output,
                                                  separate_uncond_output.get(),
                                                  model_output_is_batched_cfg,
                                                  *x,
                                                  c_out,
                                                  c_skip,
                                                  method == DDIM_TRAILING_SAMPLE_METHOD ? "gpu_ddim_trailing_denoised" : "gpu_tcd_denoised");
                if (denoised == nullptr || denoised->empty()) {
                    LOG_ERROR("GPU %s sampler backend path failed denoised tensor at step %d", sampler_label, i + 1);
                    return nullptr;
                }
                ...
                const float alpha_prod_t = static_cast<float>(ddim_alphas_cumprod[ddim_timestep]);
                const float beta_prod_t = 1.0f - alpha_prod_t;
                const float sqrt_sigma2p1 = std::sqrt(model_sigma * model_sigma + 1.0f);
                const float sqrt_alpha_t = std::sqrt(alpha_prod_t);
                const float sqrt_beta_t = std::sqrt(beta_prod_t);
                const float eps_x_coeff = 1.0f / model_sigma;
                const float eps_den_coeff = -1.0f / model_sigma;
                const float pred_x_coeff =
                    (1.0f / sqrt_sigma2p1 - sqrt_beta_t / model_sigma) / sqrt_alpha_t;
                const float pred_den_coeff = sqrt_beta_t / (model_sigma * sqrt_alpha_t);

                if (method == DDIM_TRAILING_SAMPLE_METHOD) {
                    const float alpha_prod_t_prev =
                        static_cast<float>(ddim_prev_timestep >= 0 ? ddim_alphas_cumprod[ddim_prev_timestep] : ddim_alphas_cumprod[0]);
                    const float beta_prod_t_prev = 1.0f - alpha_prod_t_prev;
                    const float variance = (beta_prod_t_prev / beta_prod_t) *
                                           (1.0f - alpha_prod_t / alpha_prod_t_prev);
                    const float std_dev_t = eta * std::sqrt(std::max(variance, 0.0f));
                    const float direction_scale =
                        std::sqrt(std::max(1.0f - alpha_prod_t_prev - std_dev_t * std_dev_t, 0.0f));
                    const float x_coeff = std::sqrt(alpha_prod_t_prev) * pred_x_coeff + direction_scale * eps_x_coeff;
                    const float den_coeff = std::sqrt(alpha_prod_t_prev) * pred_den_coeff + direction_scale * eps_den_coeff;
                    next_x = latent_runner.linear2(*x,
                                                   x_coeff,
                                                   *denoised,
                                                   den_coeff,
                                                   n_threads,
                                                   "gpu_ddim_trailing_update");
                    if (next_x != nullptr && !next_x->empty() && eta > 0.0f && std_dev_t > 0.0f) {
                        auto noise = make_step_noise(i, "gpu_ddim_trailing_step_noise");
                        if (noise == nullptr || noise->empty()) {
                            LOG_ERROR("GPU DDIM trailing sampler backend path failed CUDA noise at step %d", i + 1);
                            return nullptr;
                        }
                        next_x = latent_runner.add_scaled(*next_x,
                                                          *noise,
                                                          std_dev_t,
                                                          n_threads,
                                                          "gpu_ddim_trailing_add_noise");
                    }
                } else {
                    const float alpha_prod_t_prev =
                        static_cast<float>(ddim_prev_timestep >= 0 ? ddim_alphas_cumprod[ddim_prev_timestep] : ddim_alphas_cumprod[0]);
                    const float alpha_prod_s = static_cast<float>(ddim_alphas_cumprod[tcd_timestep_s]);
                    const float beta_prod_s = 1.0f - alpha_prod_s;
                    const float x_coeff = std::sqrt(alpha_prod_s) * pred_x_coeff + std::sqrt(beta_prod_s) * eps_x_coeff;
                    const float den_coeff = std::sqrt(alpha_prod_s) * pred_den_coeff + std::sqrt(beta_prod_s) * eps_den_coeff;
                    next_x = latent_runner.linear2(*x,
                                                   x_coeff,
                                                   *denoised,
                                                   den_coeff,
                                                   n_threads,
                                                   "gpu_tcd_update");
                    if (next_x != nullptr && !next_x->empty() && eta > 0.0f && i != steps - 1) {
                        const float ratio = alpha_prod_s != 0.0f ? alpha_prod_t_prev / alpha_prod_s : 1.0f;
                        const float carry = std::sqrt(std::max(ratio, 0.0f));
                        const float noise_scale = std::sqrt(std::max(1.0f - ratio, 0.0f));
                        auto noise = make_step_noise(i, "gpu_tcd_step_noise");
                        if (noise == nullptr || noise->empty()) {
                            LOG_ERROR("GPU TCD sampler backend path failed CUDA noise at step %d", i + 1);
                            return nullptr;
                        }
                        next_x = latent_runner.linear2(*next_x,
                                                       carry,
                                                       *noise,
                                                       noise_scale,
                                                       n_threads,
                                                       "gpu_tcd_eta_noise_update");
                    }
                }
            } else if (method == RES_2S_SAMPLE_METHOD) {
                ...
                auto denoised = make_cfg_denoised(*model_output,
                                                  ...
                                                  *x,
                                                  ...
                                                  "gpu_res_2s_denoised");
                if (denoised == nullptr || denoised->empty()) {
                    LOG_ERROR("GPU RES 2S sampler backend path failed first denoised tensor at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                auto ancestral = get_ancestral_step(sigma, sigma_next, eta);
                const float sigma_down = ancestral.first;
                const float sigma_up = ancestral.second;
                if (sigma_down == 0.0f || sigma == 0.0f) {
                    next_x = std::move(denoised);
                } else {
                    const float t = -std::log(sigma);
                    const float t_next = -std::log(sigma_down);
                    const float h = t_next - t;
                    const float a21 = c2 * phi1_fn(-h * c2);
                    const float b2 = phi2_fn(-h) / c2;
                    const float b1 = phi1_fn(-h) - b2;
                    const float sigma_c2 = std::exp(-(t + h * c2));
                    auto x2 = latent_runner.linear2(*x,
                                                    1.0f - h * a21,
                                                    *denoised,
                                                    h * a21,
                                                    ...
                                                    "gpu_res_2s_x2");
                    if (x2 == nullptr || x2->empty()) {
                        LOG_ERROR("GPU RES 2S sampler backend path failed intermediate latent at step %d", i + 1);
                        return nullptr;
                    }
                    backend_post_graphs += 1;

                    std::vector<float> scaling2 = denoiser->get_scalings(sigma_c2);
                    GGML_ASSERT(scaling2.size() == 3);
                    const float c_skip2 = scaling2[0];
                    const float c_out2  = scaling2[1];
                    const float c_in2   = scaling2[2];
                    auto noised_input2 = latent_runner.scale(*x2,
                                                             c_in2,
                                                             n_threads,
                                                             "gpu_res_2s_noised_input2");
                    if (noised_input2 == nullptr || noised_input2->empty()) {
                        LOG_ERROR("GPU RES 2S sampler backend path failed intermediate scaled input at step %d", i + 1);
                        return nullptr;
                    }
                    backend_post_graphs += 1;

                    std::vector<float> timesteps_vec2 = prepare_sample_timesteps(sigma_c2, shifted_timestep);
                    sd::Tensor<float> timesteps_tensor2({static_cast<int64_t>(timesteps_vec2.size())}, timesteps_vec2);
                    sd::Tensor<float> guidance_tensor2({1}, std::vector<float>{guidance.distilled_guidance});
                    std::unique_ptr<GgmlBackendTensorResource> model_output2;
                    std::unique_ptr<GgmlBackendTensorResource> separate_uncond_output2;
                    ...
                    if (use_dual_branch_cfg) {
                        ...
                        if (batched_noised_input2 == nullptr || batched_noised_input2->empty()) {
                            LOG_ERROR("GPU RES 2S sampler backend path failed intermediate batched input at step %d", i + 1);
                            return nullptr;
                        }
                        backend_post_graphs += 1;
                        std::vector<float> batched_timesteps_vec2;
                    ...
                    auto denoised2 = make_cfg_denoised(*model_output2,
                                                       ...
                                                       *x2,
                                                       c_out2,
                                                       c_skip2,
                                                       "gpu_res_2s_denoised2");
                    if (denoised2 == nullptr || denoised2->empty()) {
                        LOG_ERROR("GPU RES 2S sampler backend path failed second denoised tensor at step %d", i + 1);
                        return nullptr;
                    }
                    backend_post_graphs += 1;
                    next_x = latent_runner.linear3(*x,
                ...
                if (next_x != nullptr && !next_x->empty() && sigma_next > 0.0f && sigma_up > 0.0f) {
                    auto noise = make_step_noise(i, "gpu_res_2s_step_noise");
                    if (noise == nullptr || noise->empty()) {
                        LOG_ERROR("GPU RES 2S sampler backend path failed CUDA noise at step %d", i + 1);
                        return nullptr;
                    }
                    next_x = latent_runner.add_scaled(*next_x,
                                                      *noise,
                                                      sigma_up,
                                                      n_threads,
                                                      "gpu_res_2s_add_noise");
                }
            } else if (method == ER_SDE_SAMPLE_METHOD) {
                auto denoised = make_cfg_denoised(*model_output,
                                                  separate_uncond_output.get(),
                                                  model_output_is_batched_cfg,
                                                  *x,
                                                  ...
                                                  "gpu_er_sde_denoised");
                if (denoised == nullptr || denoised->empty()) {
                    LOG_ERROR("GPU ER-SDE sampler backend path failed denoised tensor at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                if (sigma_next == 0.0f) {
                    ...
                    constexpr float num_integration_points = 200.0f;
                    const int stage_used = std::min(3, i + 1);
                    const float er_lambda_s = er_lambdas[i];
                    const float er_lambda_t = er_lambdas[i + 1];
                    if (er_lambda_s <= 0.0f || er_lambda_t <= 0.0f) {
                        next_x = std::move(denoised);
                    } else {
                        const float alpha_s = sigma / er_lambda_s;
                        const float alpha_t = sigma_next / er_lambda_t;
                        const float r_alpha = alpha_t / alpha_s;
                        const float scaled_s = er_noise_scaler(er_lambda_s);
                        const float scaled_t = er_noise_scaler(er_lambda_t);
                        ...
                        next_x = latent_runner.linear2(*x,
                                                       ...
                                                       *denoised,
                                                       alpha_t * (1.0f - r),
                                                       n_threads,
                                                       "gpu_er_sde_base_update");
                        if (next_x != nullptr && !next_x->empty() && stage_used >= 2 &&
                            old_denoised != nullptr && !old_denoised->empty()) {
                            ...
                            for (int point = 0; point < static_cast<int>(num_integration_points); ++point) {
                                ...
                                const float scaled_pos = er_noise_scaler(lambda_pos);
                                if (scaled_pos != 0.0f) {
                            ...
                            auto denoised_d = latent_runner.linear2(*denoised,
                                                                    1.0f / (er_lambda_s - er_lambdas[i - 1]),
                                                                    *old_denoised,
                                                                    -1.0f / (er_lambda_s - er_lambdas[i - 1]),
                                                                    n_threads,
                                                                    "gpu_er_sde_denoised_d");
                            if (denoised_d == nullptr || denoised_d->empty()) {
                                LOG_ERROR("GPU ER-SDE sampler backend path failed denoised derivative at step %d", i + 1);
                                return nullptr;
                            }
                            backend_post_graphs += 1;
                            next_x = latent_runner.add_scaled(*next_x,
                                                              *denoised_d,
                                                              alpha_t * (dt + s * scaled_t),
                                                              n_threads,
                                                              "gpu_er_sde_second_stage_update");
                            if (next_x != nullptr && !next_x->empty() && stage_used >= 3 &&
                                er_old_denoised_d != nullptr && !er_old_denoised_d->empty()) {
                                const float denom = (er_lambda_s - er_lambdas[i - 2]) * 0.5f;
                                if (denom != 0.0f) {
                                    const float s_u = sum_u_scaled * lambda_step_size;
                                    auto denoised_u = latent_runner.linear2(*denoised_d,
                                                                            1.0f / denom,
                                                                            *er_old_denoised_d,
                                                                            -1.0f / denom,
                                                                            n_threads,
                                                                            "gpu_er_sde_denoised_u");
                                    if (denoised_u == nullptr || denoised_u->empty()) {
                                        LOG_ERROR("GPU ER-SDE sampler backend path failed denoised second derivative at step %d", i + 1);
                                        return nullptr;
                                    }
                                    backend_post_graphs += 1;
                                    next_x = latent_runner.add_scaled(*next_x,
                                                                      *denoised_u,
                                                                      alpha_t * ((dt * dt) * 0.5f + s_u * scaled_t),
                                                                      n_threads,
                                                                      "gpu_er_sde_third_stage_update");
                                }
                            }
                            er_old_denoised_d = std::move(denoised_d);
                        }
                        if (next_x != nullptr && !next_x->empty() && s_noise > 0.0f) {
                            ...
                            if (noise_scale > 0.0f) {
                                auto noise = make_step_noise(i, "gpu_er_sde_step_noise");
                                if (noise == nullptr || noise->empty()) {
                                    LOG_ERROR("GPU ER-SDE sampler backend path failed CUDA noise at step %d", i + 1);
                                    return nullptr;
                                }
                                next_x = latent_runner.add_scaled(*next_x,
                                                                  *noise,
                ...
                auto denoised = make_cfg_denoised(*model_output,
                                                  ...
                                                  *x,
                                                  ...
                                                  "gpu_heun_denoised");
                if (denoised == nullptr || denoised->empty()) {
                    LOG_ERROR("GPU Heun sampler backend path failed first denoised tensor at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                auto d = latent_runner.euler_derivative_from_denoised(*x,
                                                                      *denoised,
                                                                      sigma,
                                                                      n_threads,
                                                                      "gpu_heun_derivative_d");
                if (d == nullptr || d->empty()) {
                    LOG_ERROR("GPU Heun sampler backend path failed first derivative at step %d", i + 1);
                    return nullptr;
                }
                backend_post_graphs += 1;
                if (sigma_next == 0.0f) {
                    next_x = latent_runner.add_scaled(*x,
                                                      *d,
                                                      dt,
                                                      n_threads,
                                                      "gpu_heun_terminal_euler_update");
                } else {
                    auto x2 = latent_runner.add_scaled(*x,
                                                       *d,
                                                       dt,
                                                       n_threads,
                                                       "gpu_heun_predictor_x2");
                    if (x2 == nullptr || x2->empty()) {
                        LOG_ERROR("GPU Heun sampler backend path failed predictor latent at step %d", i + 1);
                        return nullptr;
                    }
                    backend_post_graphs += 1;
                    ...
                    auto noised_input2 = latent_runner.scale(*x2,
                                                             c_in2,
                                                             n_threads,
                                                             "gpu_heun_noised_input2");
                    if (noised_input2 == nullptr || noised_input2->empty()) {
                        LOG_ERROR("GPU Heun sampler backend path failed second scaled input at step %d", i + 1);
                        return nullptr;
                    }
                    backend_post_graphs += 1;
                    ...
                    if (use_dual_branch_cfg) {
                        ...
                        if (batched_noised_input2 == nullptr || batched_noised_input2->empty()) {
                            LOG_ERROR("GPU Heun sampler backend path failed second batched input at step %d", i + 1);
                            return nullptr;
                        }
                        backend_post_graphs += 1;
                        std::vector<float> batched_timesteps_vec2;
                    ...
                    auto denoised2 = make_cfg_denoised(*model_output2,
                                                       ...
                                                       *x2,
                                                       ...
                                                       "gpu_heun_denoised2");
                    if (denoised2 == nullptr || denoised2->empty()) {
                        LOG_ERROR("GPU Heun sampler backend path failed second denoised tensor at step %d", i + 1);
                        return nullptr;
                    }
                    backend_post_graphs += 1;
                    auto d2 = latent_runner.euler_derivative_from_denoised(*x2,
                                                                           *denoised2,
                                                                           sigma_next,
                                                                           n_threads,
                                                                           "gpu_heun_derivative_d2");
                    if (d2 == nullptr || d2->empty()) {
                        LOG_ERROR("GPU Heun sampler backend path failed second derivative at step %d", i + 1);
                        return nullptr;
                    }
                    backend_post_graphs += 1;
                    next_x = latent_runner.heun_update_from_derivatives(*x,
                                                                        *d,
                                                                        *d2,
                                                                        dt,
                                                                        n_threads,
                                                                        "gpu_heun_corrector_update");
                }
            }
            if (next_x == nullptr || next_x->empty()) {
                LOG_ERROR("GPU %s sampler backend path failed update at step %d", sampler_label, i + 1);
                return nullptr;
            }
            backend_post_graphs += 1;
            x = std::move(next_x);
...
static bool sd_version_is_base_sd1_vae_gpu_safe(SDVersion version) {
...
#ifdef SD_USE_CUDA
    if (sd != nullptr &&
        (sd_version_is_base_sd1_vae_gpu_safe(sd->version) ||
...
#ifdef SD_USE_CUDA
    return sd_version_is_base_sd1_vae_gpu_safe(sd_ctx->sd->version) ||
...
SD_API sd_latent_t* sd_sample_latent(sd_ctx_t* sd_ctx,
    ...
    int64_t t1 = ggml_time_ms();
    LOG_INFO("[Timing] sd_sample_latent latent_prepare_ms=%" PRId64 " prompt_encode_ms=%" PRId64 " denoise_ms=%" PRId64 " total_ms=%" PRId64 " init_latent=%s sampler_math_residency=cpu_tensor",
             latent_prepare_end - latent_prepare_start,
             prompt_encode_end - prompt_encode_start,
             sampling_end - sampling_start,
             t1 - t0,
             init_latent != nullptr ? "true" : "false");
...
static bool sd_sample_latent_gpu_euler_backend_true(sd_ctx_t* sd_ctx,
    ...
    if (is_supported_flow_backend) {
        ...
        plan.sample_method != DPMPP2S_A_SAMPLE_METHOD &&
        plan.sample_method != DPMPP2M_SAMPLE_METHOD &&
        plan.sample_method != DPMPP2Mv2_SAMPLE_METHOD &&
        plan.sample_method != IPNDM_SAMPLE_METHOD &&
        plan.sample_method != IPNDM_V_SAMPLE_METHOD &&
        plan.sample_method != LCM_SAMPLE_METHOD &&
        plan.sample_method != DDIM_TRAILING_SAMPLE_METHOD &&
        plan.sample_method != TCD_SAMPLE_METHOD &&
        plan.sample_method != RES_MULTISTEP_SAMPLE_METHOD &&
        plan.sample_method != RES_2S_SAMPLE_METHOD &&
        plan.sample_method != ER_SDE_SAMPLE_METHOD &&
        plan.sample_method != DPMPP_SDE_SAMPLE_METHOD &&
        plan.sample_method != DPMPP_SDE_GPU_SAMPLE_METHOD &&
        plan.sample_method != DPMPP2M_SDE_SAMPLE_METHOD &&
        plan.sample_method != DPMPP2M_SDE_GPU_SAMPLE_METHOD &&
        plan.sample_method != DPMPP2M_SDE_HEUN_SAMPLE_METHOD &&
        plan.sample_method != DPMPP2M_SDE_HEUN_GPU_SAMPLE_METHOD &&
        plan.sample_method != DPMPP3M_SDE_SAMPLE_METHOD &&
        plan.sample_method != DPMPP3M_SDE_GPU_SAMPLE_METHOD) {
    ...
    int64_t sampling_start = ggml_time_ms();
    auto resource = sd_ctx->sd->sample_kdiffusion_gpu_backend(sd_ctx,
    ...
    sd_gpu_handle_t handle = sd_gpu_register_resource(sd_ctx,
                                                      ...
                                                      SD_LAYOUT_WHCN_GGML,
                                                      output_flags,
                                                      has_gpu_init ? "sampler_latent_gpu_backend_true_init" : "sampler_latent_gpu_backend_true_t2i");
    if (handle == 0) {
...
static bool sd_sample_latent_gpu_euler_backend_bridge(sd_ctx_t* sd_ctx,
    ...
    if (plan.sample_method != EULER_SAMPLE_METHOD &&
        ...
        plan.sample_method != DPMPP3M_SDE_SAMPLE_METHOD &&
        plan.sample_method != DPMPP3M_SDE_GPU_SAMPLE_METHOD) {
        LOG_ERROR("GPU sampler backend bridge only supports the SDXL/SD1 k-diffusion sampler subset");
        return false;
    }
    ...
    sd_gpu_handle_t handle = sd_gpu_register_resource(sd_ctx,
                                                      ...
                                                      initial_noise_bridge_upload ? "sampler_latent_gpu_backend_bridge" : "sampler_latent_gpu_backend_true");
    if (handle == 0) {
...
SD_API bool sd_sample_latent_gpu(sd_ctx_t* sd_ctx,
    ...
    int64_t upload_end = ggml_time_ms();
    *out_gpu_latent = handle;
    LOG_INFO("[Timing] sd_sample_latent_gpu cpu_sample_ms=%" PRId64 " bridge_upload_ms=%" PRId64 " total_ms=%" PRId64 " bridge_upload=true",
             sample_end - t0,
             upload_end - sample_end,
             upload_end - t0);
...
SD_API bool sd_sample_latent_gpu_with_init_gpu(sd_ctx_t* sd_ctx,
    ...
    if (sd_experimental_sampler_backend_dispatch_enabled(sd_ctx)) {
        ...
        if (sd_strict_gpu_resident_enabled()) {
            LOG_ERROR("sd_sample_latent_gpu_with_init_gpu strict mode refused fallback after true GPU init-latent sampler path failed");
            return false;
        }
    }
    if (sd_strict_gpu_resident_enabled()) {
    ...
    if (ok) {
        ...
        LOG_INFO("[Timing] sd_sample_latent_gpu_with_init_gpu init_bridge_download_ms=%" PRId64 " sample_bridge_ms=%" PRId64 " total_ms=%" PRId64 " init_bridge_download=true output_bridge_upload=true",
                 download_end - t0,
...
SD_API bool sd_sample_latent_gpu_with_init_gpu_and_conditioning(sd_ctx_t* sd_ctx,
    ...
    if (sd_experimental_sampler_backend_dispatch_enabled(sd_ctx)) {
        ...
        if (sd_strict_gpu_resident_enabled()) {
            LOG_ERROR("sd_sample_latent_gpu_with_init_gpu_and_conditioning strict mode refused fallback after true GPU init-latent conditioning sampler path failed");
            return false;
        }
    }
    if (sd_strict_gpu_resident_enabled()) {
...
SD_API bool sd_estimate_vae_normal_memory(sd_ctx_t* sd_ctx,
    ...
    if (decode && sd_ctx->vae_encode_used && !sd_ctx->sd->vae_decode_only &&
        sd_model_supports_gpu_latent_decode(sd_ctx->sd->version) &&
        ...
        sd_vae_memory_report_t bridge_report;
        sd_vae_memory_report_init(&bridge_report);
        if (!sd_estimate_vae_normal_memory(decode_ctx, width, height, decode, options, &bridge_report)) {
            return false;
        }
        std::snprintf(bridge_report.fallback_reason,
                      sizeof(bridge_report.fallback_reason),
                      "%s",
                      "Decode estimate used isolated VAE context because this context has already run VAE encode");
        if (report != nullptr) {
...
SD_API bool sd_get_vae_capabilities(sd_ctx_t* sd_ctx, sd_vae_capabilities_t* capabilities) {
    ...
    *capabilities = {};
    ...
    if (sd_ctx != nullptr && sd_ctx->sd != nullptr &&
        !(sd_version_is_base_sd1_vae_gpu_safe(sd_ctx->sd->version) ||
          sd_version_is_sdxl(sd_ctx->sd->version) ||
          sd_version_is_flux(sd_ctx->sd->version) ||
          sd_version_is_flux2(sd_ctx->sd->version) ||
          sd_version_is_z_image(sd_ctx->sd->version))) {
...
SD_API bool sd_get_gpu_capabilities(sd_ctx_t* sd_ctx, sd_gpu_capabilities_t* capabilities) {
    ...
    *capabilities = {};
    ...
    const bool supports_true_gpu_kdiffusion_sampler =
        sd_ctx != nullptr &&
        sd_ctx->sd != nullptr &&
...
SD_API sd_latent_t* sd_gpu_latent_download(sd_ctx_t* sd_ctx,
    ...
    while (!latent.empty() && latent.dim() < 4) {
        latent.unsqueeze_(latent.dim());
    }
    if (latent.empty()) {
        return nullptr;
    }
...
static bool sd_decode_vae_encoded_gpu_latent_bridge(sd_ctx_t* sd_ctx,
    ...
    const bool input_vae_encode = (resource->flags & SD_GPU_RESOURCE_FLAG_VAE_ENCODE_OUTPUT) != 0;
    ...
    std::snprintf(full_report.fallback_reason,
                  ...
                  "%s",
                  input_cpu_bridge
                      ? "Input latent was CPU-bridge-uploaded before this decode; decode used isolated VAE context with device-to-device handoff"
                      : (input_vae_encode
